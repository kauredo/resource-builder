...
import { CreateCollectionDialog } from "@/components/resource/CreateCollectionDialog";
...
export default function ResourcesPage() {
  ...
  const collections = useQuery(
    api.collections.getUserCollectionsWithThumbnails,
    user?._id ? { userId: user._id } : "skip"
  );

  const [tab, setTab] = useState<TabValue>(
    (searchParams.get("tab") as TabValue) === "collections" ? "collections" : "all"
  );
  ...
  const updateURL = useCallback(
    (newTab: TabValue) => {
      ...
      router.replace(
        `/dashboard/resources${params.toString() ? `?${params.toString()}` : ""}`,
        { scroll: false }
      );
    },
    [router, searchParams]
  ...
  const filteredCollections = useMemo(() => {
    ...
    return result;
  }, [collections, collectionSearch, collectionSort]);
  ...
  const hasAnyCollections = collections && collections.length > 0;
  const hasFilteredCollections = filteredCollections.length > 0;
  const isCollectionFiltering = collectionSearch.trim() !== "";
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      {/* Collections Tab */}
      {tab === "collections" && (
        <div role="tabpanel" id="tabpanel-collections" aria-labelledby="tab-collections">
          {hasAnyCollections && (
            <>
              {/* Search + sort */}
          ...
          {!hasAnyCollections && (
            <div className="rounded-xl border-2 border-dashed border-border/60 bg-muted/20">
              <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
                ...
                <p className="text-sm text-muted-foreground mb-5 max-w-sm leading-relaxed">
                  Group resources by therapy program, session, or however you like.
                </p>
                <Button
                  onClick={() => setShowCreateCollection(true)}
                  className="btn-coral gap-2"
          ...
          {/* No matching results */}
          {hasAnyCollections && !hasFilteredCollections && isCollectionFiltering && (
            <div className="py-16 text-center">
              <p className="text-muted-foreground mb-4">
                No collections match &ldquo;{collectionSearch}&rdquo;
              </p>
              <Button variant="outline" onClick={() => setCollectionSearch("")} size="sm">
                Clear search
              </Button>
            </div>
          )}
      ...
      {/* Add to Collection Dialog (from batch select) */}
      {user && (
        <AddToCollectionDialog
          open={showAddToCollection}
          onOpenChange={setShowAddToCollection}
          resourceIds={[...selectedIds] as Id<"resources">[]}
          userId={user._id}
        />
      )}
    </div>
